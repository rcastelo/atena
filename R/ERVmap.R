...
#' rate employed by the ERVmap algorithm to discard aligned reads whose rate
#' of sum of hard and soft clipping or whose rate of the edit distance over the 
#' genome reference to the length of the read is above this threshold.
#'
...
#' The default, \code{suboptimalAlignmentTag="auto"}, first extracts the name of 
#' the read mapper software from one or more BAM files. If BAM files were 
...
#' When this value is set to \code{NA}, the filtering step based on suboptimal
