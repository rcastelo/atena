...
Now, let's fetch the same annotations but process them using the 
`rmskatenaparser` parser function. We set the parameter `strict=FALSE` to avoid
...
Moreover, the `atena` package provides useful functions to retrieve TEs of a
...
can use the `getLINEs()` function. We use the previously obtained object
`rmskat` with annotations parsed with the `rmskatenaparser()` function, and set
the `relLength` to 0.9.
...
quantification. As in the case of the ERVmap method from `atena`, either
secondary alignments or the `NH` tag are required for gene expression
quantification.
...
For paired-end data (`singleEnd=FALSE`), the usage of the `fragments` argument
is the same as in `TelescopeParam()`.
...
only the elements with "type" = "exon" are considered for the analysis. Then,
exon counts are summarized to the gene level in a `GRangesList` object. This
also applies to the ERVmap and Telescope methods for `atena` when gene features
are present. Let's see an example of this processing:
...
```{r}
features_tt <- features(ttpar_gen)
features_tt[!mcols(features_tt)$isTE]
```
...
`qtex()` method returns a `SummarizedExperiment` object containing the
