...
In this package, we include annotations for 3,220 human ERVs (HERVs) (a type of class I TEs) collected by ERVmap authors [@tokuyama2018ervmap], available [here](https://github.com/mtokuyama/ERVmap/blob/master/ERVmap.bed) (GRCh38/hg38 genome assembly). Moreover, this packaga also contains annotations of TEs (HERVs and L1 long interspersed nuclear element (LINE)) created by Telescope authors [@bendall2019telescope] for the GRCh38/hg38 human genome assembly (28,513 TEs) and for the GRCh37/hg19 assembly (28,314 TEs), available [here](https://github.com/mlbendall/telescope_annotation_db).
...
bamfiles <- list.files(system.file("extdata", package="atena"),
                       pattern="*.bam", full.names=TRUE)
annot <- ERVmap_ann()
ttpar <- TEtranscriptsParam(bamfiles, annot, singleEnd = TRUE, ignoreStrand=TRUE)
