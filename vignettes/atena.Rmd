...
The dataset that will be used to quantify TE expression is a published RNA-seq dataset of _Drosophila_ available at the National Center for Biotechnology Information (NCBI) Gene Expression Omnibus (accession no. GSE47006, https://www.ncbi.nlm.nih.gov). The two selected samples are: piwi knockdown and piwi control (GSM1142845 and GSM1142844). The piwi-associated silencing complex (piRISC) silences TEs in the _Drosophila_ ovary, thus, the knockdown of piwi causes the de-repression of TEs.
...
Regarding gene expression quantification, `atena` has implemented the approach of the original TEtranscripts software [@jin2015tetranscripts]. As in the case of the ERVmap and Telescope methods from `atena`, either secondary alignments or the `NH` tag are required for gene expression quantification.
...
As mentioned, TE expression quantification is provided at the repeat name level. All 28 TE features share the same repeat name. This, the expression levels of the initial 28 TE features have been a aggregated into 1 element with repeat name _ROO_LTR_.
